module bin2bcd (
    ...
    wire [19:0] w_in_bcd; //Salida de suma
    wire [19:0] out_bcd;
    ...
    assign w_mux_in1 = 4'b0011; //3
    assign w_mux_in2 = 4'b1011; //-5

    assign w_MSB={w_in_bcd[19],w_in_bcd[15],w_in_bcd[11],w_in_bcd[7],w_in_bcd[3]}; //Bits mas significativos de cada nibble
    assign Result = {w_decmil, w_mil, w_cent, w_dec, w_unit};
    ...
    sum sum0(
        .A(w_mux_out[3:0]),
        .B(w_unit),
        .sum_out(w_in_bcd[3:0])
    );
    sum sum1(
        .A(w_mux_out[7:4]),
        .B(w_dec),
        .sum_out(w_in_bcd[7:4])
    );
    sum sum2(
        .A(w_mux_out[11:8]),
        .B(w_cent),
        .sum_out(w_in_bcd[11:8])
    );
    sum sum3(
        .A(w_mux_out[15:12]),
        .B(w_mil),
        .sum_out(w_in_bcd[15:12])
    );
    sum sum4(
        .A(w_mux_out[19:16]),
        .B(w_decmil),
        .sum_out(w_in_bcd[19:16])
    );
